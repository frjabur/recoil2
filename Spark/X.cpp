...
{
	//PlaySound(TEXT("C:\\Users\\fejab\\Desktop\\bless.wav"), NULL, SND_FILENAME);
	char szNewTitle[MAX_PATH];
	sprintf(szNewTitle, "X", globals.WeaponName);
	if (globals.paused)
	{
...
void AsyncVariableLoop()
{
	while (!globals.quit)
	{
		...
		{
			Sleep(5);//	ResetCursorPos((5.0f / 1000.0f)); //Change to the value of Sleep
		}
		Sleep(5);
	}
}
...
__forceinline void RunFrame()
{
	if (!globals.recording)
	{
		...
		{
			//Idle
//			if (GET_CONVAL_INT(&enable_quick_record) && !globals.InChangeWeaponOverlay)
//			{
//				DrawCenterCrosshair();
//			}
		}
