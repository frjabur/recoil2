...
void ListenForHotkeys()
{
					...
					if (globals.InChangeWeaponOverlay)
					{
						...
						else if (GetAsyncKeyState(VK_RETURN) >> 8)
						{
							...
							if (globals.ChangeWeaponOverlayTextLength > 0)
							{
								...
								if (ReadWeaponConfig())
								{
									RenderTimedOverlay(0.7, XY(100, 75), "SUCCESS!", RGB(0, 255, 0), 1, DEFAULT_FONT);
								}
								else
								{
									RenderTimedOverlay(0.7, XY(100, 75), "ERROR!", RGB(255, 0, 0), 1, DEFAULT_FONT);
								}
							}
						...
						else
						{
							...
							DrawTextOverlay(XY(95, 70), OverlayText, RGB(0, 0, 0), 1, DEFAULT_FONT); //Black
							DrawTextOverlay(XY(97, 72), OverlayText, RGB(136, 0, 21), 1, DEFAULT_FONT); //Dark Red
							DrawTextOverlay(XY(99, 74), OverlayText, RGB(255, 0, 0), 1, DEFAULT_FONT); //Red
						}
					}
	...
	{
		TryLoadingScriptFile("ump");
	}
	if (GetAsyncKeyState(VK_F4))
	{
		TryLoadingScriptFile("mac10");
	}
	if (GetAsyncKeyState(VK_F5))
	{
		TryLoadingScriptFile("mp9");
	}
	if (GetAsyncKeyState(VK_F6))
	{
		TryLoadingScriptFile("famas");
	}
	if (GetAsyncKeyState(VK_F7))
	{
		TryLoadingScriptFile("galil");
	}
	if (GetAsyncKeyState(VK_LMENU))
	{
		TryLoadingScriptFile("OFF");
	}
	if (GetAsyncKeyState('C'))
	{
		TryLoadingScriptFile("ak800z");
	}
	if (GetAsyncKeyState('Z'))
	{
		TryLoadingScriptFile("m4a160z");
	}
	if (GetAsyncKeyState('X'))
	{
		TryLoadingScriptFile("cz75");
	}
	if (GetAsyncKeyState('V'))
	{
		TryLoadingScriptFile("m4a4960z");
	}
