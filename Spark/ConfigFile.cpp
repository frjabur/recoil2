...
int ReadConfigFile()
{
	...
	if (strstr(tempString, "X Config Version"))
	{
		...
		{
			printf("Error: X.cfg is newer (%f) than this mIRC revision supports (%f)!\n", globals.ConfigVersion, CONFIG_FILE_VERSION);
			delete[] tempString;
			configFile.close();
			return 0;
		}
	...
	while (strlen(tempString) != 0)
	{
		...
		{
			//invalid
			DEBUG_PRINT("Found Invalid line in mIRC.cfg");
		}
		delete command;
		tempString[0] = 0;
		configFile.getline(tempString, 1024);
...
int WriteConfigFile(void)
{
	...
	configFile << "mIRC Config Version " << CONFIG_FILE_VERSION << std::endl;
